      ...
      >
        ...
        <template v-else-if="!loading && searchKeyword && agentList?.length === 0">
          <view class='empty-state'>
            ...
            />
            <text class='empty-text'>{{ t("Mobile.AgentSearch.emptyResult") }}</text>
          </view>
        </template>
...
<script lang="ts" setup>
  ...
  import RecentUsedAgentItem from '@/components/recent-used-agent-item/recent-used-agent-item.vue'
  ...
  const searchPlaceholderSource = ref<string>(
    "Mobile.AgentSearch.placeholder",
  );
      ...
      if (app.globalData && app.globalData.localAgentList && Array.isArray(app.globalData.localAgentList) && app.globalData.localAgentList.length > 0) {
        ...
        if (app.globalData.searchPlaceholder) {
          searchPlaceholderSource.value =
            app.globalData.searchPlaceholder as string
        }
      } else {
        // 没有数据，回退到 api 模式
