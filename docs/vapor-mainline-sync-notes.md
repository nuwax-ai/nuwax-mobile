...
## 二、两线逻辑差异现状（2026-08-19 已核实）

**会话详情方案本体（`agent-detail.uvue`）无逻辑分叉**：与主线（`1ff8cda8` 引入）同源；vapor 侧仅多一处**附加性**增强——evalJS 失败时兜底走元素级 `UniWebViewElement.evalJS`（非 vapor 运行时主路成功、兜底永不触发，行为与主线完全一致），另加 1 行 `[AgentDetailWebView]` 地址诊断日志。

**真实行为差异清单**：

| 差异 | 方向 | 影响 |
|---|---|---|
| mermaid 预览关闭只展示源码（`8ead8824`） | vapor 独有 | App 会话内 mermaid 不出图（vapor evalJS 限制）；H5/小程序端不受影响 |
| ai-msg 工具卡 name 换行清洗（`5d7b8e91`） | 主线未合入 | 合并后生效，回归重点 |
| voice TTS 连接错误/停滞检测、尾部容错（`35b4e416` / `f9315816`） | 主线未合入 | 合并后经 vapor 壳层语音桥透传生效，需回归 |
| 样式规约改造：反嵌套 BEM 等（`b9270ca7`） | vapor 独有 | 视觉等价的等价改写，无行为差异 |

聊天链路共 31 文件 ±3980 行差异，均为上述双向提交叠加；渲染层改动（ScrollManager 重构、scroll-view 全量渲染）来自 native-math→vapor 合并系列，主线侧无对应冲突提交。
