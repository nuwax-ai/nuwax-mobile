      ...
      <view class="plan-header">
        <view class="plan-info">
          <text class="plan-name">{{
            planNameDisplay ||
            getI18nText("Mobile.ThirdParty.MpHtml.executionPlan")
          }}</text>
    ...
    <template v-else>
      ...
      <view class="tool-header" @tap="toggleExpanded">
        <view class="tool-info">
          <view
            v-if="primaryFileDiff"
            class="file-tool-title"
            @tap.stop.prevent="toggleFileDiffExpanded"
          >
            <text class="file-tool-title-text">{{ fileToolTitleDisplay }}</text>
            <view class="file-tool-stats">
              <text class="file-tool-added">+{{ fileDiffStats.added }}</text>
              <text class="file-tool-removed"
                >-{{ fileDiffStats.removed }}</text
              >
            </view>
          </view>
          <text v-else class="tool-name">{{ toolNameDisplay }}</text>
          <view class="tool-status-display">
            <view
              class="status-icon"
              :class="getStatusIconClass(toolCall.status)"
            >
              <text
                :class="getStatusIconType(toolCall.status)"
                :style="{
                  color: getStatusIconColor(toolCall.status),
                  fontSize: '32rpx',
                }"
              ></text>
            </view>
            <text class="tool-status-text">{{
        ...
        <view class="tool-actions">
          <view
            class="action-icon"
            @tap.stop.prevent="handleShowDetails(detailData)"
          >
          ...
          <view
            v-if="isPageType(toolCall)"
            class="action-icon"
            @tap.stop.prevent="openPreviewPage(toolCall)"
          >
      ...
      <view v-if="primaryFileDiff && fileDiffExpanded" class="file-diff-inline">
        <diff-content-view :diffLinesList="primaryFileDiffLines" />
      </view>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isPlanType() {
        return (
          this.toolCall.type === AgentComponentTypeEnum.Plan ||
          this.toolCall.type === "Plan"
        );
      },
      ...
      fileDiffItems() {
        return normalizeFileDiffItems(this.toolCall.result || this.toolCall);
      },

      primaryFileDiff() {
        return this.fileDiffItems.length > 0 ? this.fileDiffItems[0] : null;
      },

      fileDiffStats() {
        return getFileDiffTotalStats(this.fileDiffItems);
      },

      primaryFileDiffLines() {
        const file = this.primaryFileDiff;
        return file ? getFileDiffLines(file) : [];
      },

      fileToolTitleDisplay() {
        const file = this.primaryFileDiff;
        if (!file) {
          return "";
        }
        const name = this.toolNameDisplay;
        if (!name) {
          return file.path;
        }
        if (name.indexOf(file.path) >= 0) {
          return name;
        }
    ...
    methods: {
      ...
      async toggleExpanded() {
        if (this.primaryFileDiff) {
          this.toggleFileDiffExpanded();
          return;
        }
        ...
        if (result?.kind === "edit") {
          ...
          const fileProxyUrl = await getFileProxyUrlByConversationIdAndFilePath(
            conversationId,
            file_path,
          );
      ...
      toggleFileDiffExpanded() {
        this.fileDiffExpanded = !this.fileDiffExpanded;
      },
