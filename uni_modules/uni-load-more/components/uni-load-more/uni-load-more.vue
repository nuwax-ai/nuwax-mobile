...
<script>
	...
	export default {
		...
		data() {
			...
			computed: {
				iconSnowWidth() {
					const baseSize = Math.floor(this.iconSize / 24)
					const fixedSize = baseSize > 0 ? baseSize : 1
					return fixedSize * 2
				},
				contentdownText() {
					const text = this.contentText.contentdown
					if (typeof text === 'string' && text.length > 0) {
						return text
					}
					return translate("uni-load-more.contentdown")
				},
				contentrefreshText() {
					const text = this.contentText.contentrefresh
					if (typeof text === 'string' && text.length > 0) {
						return text
					}
					return translate("uni-load-more.contentrefresh")
				},
				contentnomoreText() {
					const text = this.contentText.contentnomore
					if (typeof text === 'string' && text.length > 0) {
						return text
					}
					return translate("uni-load-more.contentnomore")
				}
			},
		mounted() {
			// #ifdef APP
