...
3. **签名材料**（已备好，`~/Downloads/Nuwa iOS 证书/`，p12 密码 `Admin123`）：
   - 开发证书 + `Nuwa_iOS_Dev.mobileprovision`（AppID `89GQ2RJVW7.com.nuwax.nuwa`，含 HotspotConfiguration entitlement）。
...
- **签名（手动）**：bundle id `com.nuwax.nuwa`、`DEVELOPMENT_TEAM = 89GQ2RJVW7`、
...
2. App 扫码（开发二维码带开发 PoP `Nuwax7-Dev-PoP`）→ 直连（30s 窗口）→
